...
download(unihanUrl, dist, dl_options)
  .then((data) => {
    console.log(chalk.blue("Downloading Unihan database..."));
    console.log(chalk.green("Done!"));
    return data
      .find((ele) => ele.path == "Unihan_DictionaryLikeData.txt")
      .data.toString("utf-8");
  })
  .then((data) => {
...
fs.exists("data", (exists) => {
  if (!exists) {
    ...
    gitP()
      .clone("http://git.chise.org/git/chise/ids.git", workPath)
      .then(genIDSRaw)
      .then(genIDSdata);
...
function genIDSdata(data) {
  ...
  console.log("writing idsraw")
  fs.writeFileSync(
    "data/idsraw.js",
    JSON.stringify(ids_parts),
    "utf-8"
  );
  for (let key in ids_parts) gen_decompose(key, ids_parts[key]);
  inverted.map(function (decompose, idx) {
    fs.writeFileSync(
      "data/idsdata/decompose" + idx + ".js",
      "module.exports=" + JSON.stringify(decompose, "", " "),
      "utf8"
    );
  });
...
function genStrokeMap(charstroke) {
  ...
  for (var i = 0; i < mapbmp.length; i++) {
    if (bmpstr.charCodeAt(i) - 0x23 !== mapbmp[i]) {
      console.log(
        "bmp string conversion error at ",
        i,
        bmpstr.charCodeAt(i),
        mapbmp[i]
      );
...
var gen_decompose = function (glyph, parts) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (ids_parts[part] && part != glyph) {
      //work around for u+2c8ab
      depth++;
